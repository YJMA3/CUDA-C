...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    //int rowblocks = M.height/16;
    //int columnblocks = N.width/16;
	//if(M.height % 16){rowblocks++;}
	//if(N.width % 16){columnblocks++;}
	dim3 dimGrid((TILE_WIDTH + N.width-1)/TILE_WIDTH, (TILE_WIDTH + M.height-1)/TILE_WIDTH, 1);
