...
int main(int argc, char* argv[])
{
    ...
    //cudaMalloc((void **)&input_data, INPUT_HEIGHT * INPUT_WIDTH * sizeof(uint32_t));
    //cudaMalloc((void **)&input_bins, INPUT_HEIGHT * INPUT_WIDTH * sizeof(uint32_t));
    uint32_t *input_data = (uint32_t *)AllocateDeviceMemory(INPUT_WIDTH, INPUT_HEIGHT, sizeof(uint32_t));
    uint32_t *input_bins = (uint32_t *)AllocateDeviceMemory(HISTO_WIDTH,HISTO_HEIGHT, sizeof(uint32_t));
    

    cudaMemset(input_bins , 0, HISTO_WIDTH * HISTO_HEIGHT * sizeof(uint32_t));

    // copy the inputs from host to device
    CopyToDevice(input_data, &input[0][0], INPUT_HEIGHT, INPUT_WIDTH, sizeof(uint32_t));  
    ...
    TIME_IT("opt_2dhisto",
            50,
            opt_2dhisto(input_data, input_bins);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */

	CopyToHost(kernel_bins, input_bins, HISTO_WIDTH * HISTO_HEIGHT * sizeof(uint32_t));  
