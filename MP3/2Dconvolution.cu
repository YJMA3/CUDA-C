...
int main(int argc, char** argv) {
	...
	cudaEventElapsedTime(&cpu_milliseconds, cpu_start, cpu_stop);
	
	printf("The total elapsed time of CPU is:  %f seconds\n", cpu_milliseconds);
...
void ConvolutionOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	dim3 dimGrid((BLOCK_SIZE + N.width-1)/TILE_SIZE, (BLOCK_SIZE + N.height-1)/TILE_SIZE, 1);
	...
	printf("The total elapsed time of GPU is:  %f seconds\n", gpu_milliseconds);
	...
	//printf("The total elapsed time of Convolution is:  %f seconds\n", milliseconds);
	printf("The total elapsed time of Overheads is:  %f seconds\n", milliseconds-gpu_milliseconds);
